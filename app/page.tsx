...
const experiences = [
  {
    ...
    points: ["Currently responsible for ensuring the quality and reliability of the One Statement project"],
  },
  {
    ...
    <main className="min-h-screen font-mono" style={{ backgroundColor: "var(--bg)", color: "var(--text-primary)" }}>
      ...
      <section className={sectionClass} style={sectionStyle}>
        <div className="max-w-5xl mx-auto px-6 pt-24 pb-12 md:pt-28 md:pb-16">
          <div className="flex flex-col sm:flex-row items-start gap-6 sm:gap-8">
            <div className="w-20 h-20 md:w-24 md:h-24 rounded-full flex-shrink-0 overflow-hidden" style={{ border: "1px solid var(--border)" }}>
              <img src="/gallery/avatar-1.jpeg" alt="Avatar" className="w-full h-full object-cover" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-1">
                <h1 className="text-xl md:text-2xl font-bold tracking-tight flex items-center gap-2 flex-wrap" style={{ color: "var(--text-heading)" }}>
                  Alfito Febriansyah
                  <svg className="w-[1.1rem] h-[1.1rem] text-blue-400 flex-shrink-0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              ...
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                <span className="text-[10px] uppercase tracking-widest px-1.5 py-0.5" style={{ color: "var(--text-muted)", border: "1px solid var(--border)" }}>Software Engineer</span>
                <span className="text-[10px] uppercase tracking-widest px-1.5 py-0.5" style={{ color: "var(--text-muted)", border: "1px solid var(--border)" }}>Quality Assurance</span>
              </div>
      ...
      <section className={sectionClass} style={sectionStyle}>
        <div className={innerClass}>
          <h2 className="text-xs tracking-widest uppercase mb-6 md:mb-8" style={headingStyle}>// Tech Stack</h2>
          <TechStack />
        </div>
      </section>
              ...
              <div key={i} className="flex flex-col sm:grid sm:grid-cols-12 gap-2 sm:gap-6 py-6">
                <div className="sm:col-span-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-[10px] tabular-nums" style={{ color: "var(--text-secondary)" }}>{exp.period}</p>
                    <span className="text-[10px] tabular-nums px-1.5 py-0.5" style={{ color: "var(--text-muted)", border: "1px solid var(--border)" }}>
                      {calcDuration(exp.period)}
                    </span>
                  </div>
                  <p className="text-[10px] mt-1" style={{ color: "var(--text-muted)" }}>{exp.location}</p>
                </div>
                <div className="sm:col-span-9">
                  <h3 className="text-sm font-semibold mb-0.5" style={{ color: "var(--text-heading)" }}>{exp.role}</h3>
                  <p className="text-xs mb-3" style={{ color: "var(--text-secondary)" }}>@ {exp.company}</p>
        ...
        <div className={innerClass}>
          ...
          <div className="divide-y divide-[var(--border)]">
            {educations.map((edu, i) => (
              <div key={i} className="flex flex-col sm:grid sm:grid-cols-12 gap-2 sm:gap-6 py-6">
                <div className="sm:col-span-3">
                  <p className="text-[10px] tabular-nums" style={{ color: "var(--text-secondary)" }}>{edu.period}</p>
                  <p className="text-[10px] mt-1" style={{ color: "var(--text-muted)" }}>{edu.location}</p>
                </div>
                <div className="sm:col-span-9">
                  <h3 className="text-sm font-semibold mb-0.5" style={{ color: "var(--text-heading)" }}>{edu.degree}</h3>
                  <p className="text-xs mb-1" style={{ color: "var(--text-secondary)" }}>@ {edu.school}</p>
                  <p className="text-[10px] mb-3" style={{ color: "var(--text-muted)" }}>GPA {edu.gpa}</p>
                  <ul className="space-y-1.5">
        ...
        <div className="max-w-5xl mx-auto px-6 py-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
          ...
          <div className="flex items-center gap-4">
            {[
              ...
              { name: "linkedin", url: "https://linkedin.com/in/alfito-fbriansyah" },
            ].map((s) => (
